...
function buildDateRangeClause({ startDate, endDate, params, column = 'wr.wasted_at' }) {
  let clause = '';

  if (startDate) {
    params.push(startDate);
    clause += ` AND DATE(${column}) >= $${params.length}`;
  }
...
router.get('/', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    ...
    queryText += buildDateRangeClause({ startDate, endDate, params });
...
router.get('/summary', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    ...
    let filteredRangeClause = '';
    if (startDate || endDate) {
      filteredRangeClause = buildDateRangeClause({ startDate, endDate, params, column: 'filtered.wasted_at' });
    }
    ...
    const queryText = `
      SELECT
         ...
         (
           SELECT COALESCE(SUM(filtered.total_loss), 0)
           FROM waste_records filtered
           WHERE ($1::int IS NULL OR filtered.location_id = $1)
           ${filteredRangeClause}
         ) AS filtered_loss,
         (
           SELECT COUNT(*)
    ...
    res.json({
      ...
      filtered_loss: Number(summary.filtered_loss || 0),
