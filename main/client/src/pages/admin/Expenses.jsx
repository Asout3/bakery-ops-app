...
export default function ExpensesPage() {
  ...
  return (
    <div className="expenses-page">
      ...
      <div className="card expenses-filter-card mb-4">
        <div className="card-body">
          <div className="expenses-filter-row">
            ...
            <button className="btn btn-outline-secondary btn-sm expenses-clear-filter-btn" onClick={() => setSelectedDay('')}>
              Clear Day Filter
            </button>
          </div>
        </div>
      ...
      <div className="expenses-metrics-grid mb-4">
        <div className="card expenses-metric-card"><div className="expenses-metric-icon tone-danger"><TrendingDown size={20} /></div><div className="expenses-metric-content"><h3>ETB {totalExpenses.toFixed(2)}</h3><p>Total Expenses</p></div></div>
        <div className="card expenses-metric-card"><div className="expenses-metric-icon tone-warning"><DollarSign size={20} /></div><div className="expenses-metric-content"><h3>{expenses.length}</h3><p>Total Records</p></div></div>
        <div className="card expenses-metric-card"><div className="expenses-metric-icon tone-info"><Calendar size={20} /></div><div className="expenses-metric-content"><h3>ETB {averageExpense.toFixed(2)}</h3><p>Avg. Expense</p></div></div>
      </div>

      <div className="card expenses-table-card"><div className="card-body"><div className="table-responsive"><table className="table table-hover"><thead><tr><th>Expense ID</th><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Created By</th><th>Edit Window</th><th>Actions</th></tr></thead><tbody>
        {expenses.length === 0 ? <tr><td colSpan={8} className="text-center">No expenses found</td></tr> : expenses.map((expense) => (
          <tr key={expense.id}><td>{expense.expense_code || `EXP-${String(expense.id).padStart(6, '0')}`}</td><td>{new Date(expense.expense_date).toLocaleDateString()}</td><td><span className="badge badge-primary">{expense.category}</span></td><td><button className="btn btn-sm btn-outline-secondary" onClick={() => setShowDescription({ ...expense, description: typeof expense.description === 'string' ? expense.description : '' })}><Eye size={14} /> View</button></td><td><strong>ETB {Number(expense.amount).toFixed(2)}</strong></td><td>{expense.created_by_name || '-'}</td><td>{isEditable(expense) ? <span className="badge badge-warning"><Clock size={12} className="me-1" />{getMinutesRemaining(expense)}m left</span> : <span className="badge badge-secondary">Locked</span>}</td><td><button className="btn btn-sm btn-outline-primary me-2" onClick={() => { if (!isEditable(expense)) return; setEditingExpense(expense); setFormData({ category: expense.category, description: expense.description || '', amount: expense.amount, expense_date: isAdmin ? expense.expense_date : today }); setShowForm(true); }} disabled={!isEditable(expense)}><Edit size={14} /></button><button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(expense.id)} disabled={!isEditable(expense)}><Trash2 size={14} /></button></td></tr>
        ))}
