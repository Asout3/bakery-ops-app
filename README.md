...
If GitHub shows **Unable to merge**, validate this sequence:

1. Confirm your PR base branch is correct (`main` vs `master`).
2. Update your PR branch with the latest base branch:
   - `git fetch origin`
   - `git rebase origin/<base-branch>` (or `git merge origin/<base-branch>`)
   - resolve conflicts, then push (`git push --force-with-lease` after rebase).
3. Verify all required checks are green (test/build/lint).
4. Verify required reviewers/approvals are complete.
5. If branch protection requires a specific strategy, use the repository’s configured merge mode from the PR UI (merge/squash/rebase).
