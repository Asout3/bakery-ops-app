...
export default function StaffPaymentsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      payroll_month: formData.payout_mode === 'pay_to_month' ? formData.payroll_month : null,
    ...
    if (payload.payout_mode === 'pay_to_month' && !payload.payroll_month) {
      setFeedback({ type: 'warning', message: 'Payroll month is required when payout mode is Pay to Month.' });
      return;
    }
        ...
        <div className="card-body table-responsive">
          <table className="table table-hover">
            ...
            <tbody>
              ...
              {!filteredPayments.length && <tr><td colSpan="10" className="text-center text-muted">No payments match the current filters.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
          ...
          <div className="modal-content" style={{ maxWidth: '1200px', width: '96vw' }} onClick={(e) => e.stopPropagation()}>
            ...
            <form className="modal-body" onSubmit={handleSubmit}>
              <div className="row g-3">
                {suggestedPayment && <div className="col-12"><div className="alert alert-info" style={{ display: "flex", flexWrap: "wrap", gap: "0.65rem", alignItems: "center", justifyContent: "space-between" }}><div style={{ fontSize: "0.95rem" }}>Worked days since last payment: <strong>{suggestedPayment.workedDays}</strong> • Daily rate: <strong>ETB {suggestedPayment.dailyRate.toFixed(2)}</strong> • Suggested payout: <strong>ETB {suggestedPayment.recommendedAmount.toFixed(2)}</strong></div><button type="button" className="btn btn-outline-primary" onClick={() => setFormData((prev) => ({ ...prev, amount: suggestedPayment.recommendedAmount.toFixed(2) }))}>Use Suggested</button></div></div>}
                ...
                <div className="col-md-4"><label className="form-label">Payout *</label><select className="form-select" value={formData.payout_mode} onChange={(e) => setFormData({ ...formData, payout_mode: e.target.value, payroll_month: e.target.value === 'pay_to_month' ? formData.payroll_month : '' })}><option value="pay_now">Pay Now</option><option value="pay_to_month">Pay to Month</option></select></div>
                <div className="col-md-4"><label className="form-label">Payroll Month</label><input type="month" className="form-control" value={formData.payroll_month} onChange={(e) => setFormData({ ...formData, payroll_month: e.target.value })} disabled={formData.payout_mode !== 'pay_to_month'} required={formData.payout_mode === 'pay_to_month'} /></div>
