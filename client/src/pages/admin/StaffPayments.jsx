...
export default function StaffPaymentsPage() {
  ...
  const handleStaffSelect = (staffId) => {
    ...
    const staffPayments = payments.filter((payment) => Number(payment.staff_profile_id) === Number(staffId)).sort((a, b) => new Date(b.payment_date) - new Date(a.payment_date));
            ...
            <tbody>
              {payments.map((payment) => (
                <tr key={payment.id}>
                  ...
                  <td><button className="btn btn-sm btn-outline-secondary" onClick={() => setNotePreview(payment.notes || "") }><Eye size={14} /> View</button></td><td>
          ...
          <div className="modal-content" style={{ maxWidth: '900px', width: '95vw' }} onClick={(e) => e.stopPropagation()}>
            ...
            <form className="modal-body" onSubmit={handleSubmit}>
              <div className="row g-3">
                {suggestedPayment && <div className="col-12"><div className="alert alert-info">Worked days since last payment: <strong>{suggestedPayment.workedDays}</strong> • Daily rate: <strong>ETB {suggestedPayment.dailyRate.toFixed(2)}</strong> • Suggested payout: <strong>ETB {suggestedPayment.recommendedAmount.toFixed(2)}</strong> <button type="button" className="btn btn-sm btn-outline-primary ms-2" onClick={() => setFormData((prev) => ({ ...prev, amount: suggestedPayment.recommendedAmount.toFixed(2) }))}>Use Suggested</button></div></div>}
                ...
                <div className="col-md-8"><label className="form-label">Notes</label><textarea rows="4" className="form-control" style={{ minHeight: "120px" }} value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} /></div>
              </div>
