        ...
        <div className="card-body" style={productsControlsRowStyle}>
          ...
          <div>
            ...
            <div className="d-flex gap-2" style={{ minWidth: '320px' }}>
              <input className="form-control" placeholder={t('newCategory')} value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} />
          ...
          <div className="card-header d-flex justify-content-between align-items-center" style={{ gap: '0.75rem', flexWrap: 'wrap' }}>
            {groupRename.name === groupName ? (
              <div className="d-flex gap-2" style={{ flex: 1, maxWidth: '520px' }}>
                ...
                <button className="btn btn-sm btn-primary" onClick={() => handleRenameGroup(groupName)} disabled={saving}>{t('save')}</button>
                <button className="btn btn-sm btn-secondary" onClick={() => setGroupRename({ name: '', next: '' })}>{t('cancel')}</button>
              </div>
            ) : (
          ...
          <div className="modal-content" style={{ maxWidth: '1200px', width: '95vw' }} onClick={(e) => e.stopPropagation()}>
            ...
            {showGroupForm && (
              <form onSubmit={handleCreateGroup} className="modal-body">
                ...
                <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={resetForms}>{t('cancel')}</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Create Group'}</button></div>
              </form>
            )}
            {showVariantForm && (
              <form onSubmit={handleCreateVariant} className="modal-body">
                ...
                <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={resetForms}>{t('cancel')}</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Add Variant'}</button></div>
              </form>
            )}
            {editingProduct && (
              <form onSubmit={handleEditSubmit} className="modal-body">
                ...
                <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={resetForms}>{t('cancel')}</button><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Update Variant'}</button></div>
              </form>
            )}
          </div>
        </div>
