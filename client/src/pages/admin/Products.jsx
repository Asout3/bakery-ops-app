...
const emptyVariant = { name: '', price: '', cost: '', unit: 'piece', source: 'baked', category_id: '', low_stock_threshold: '' };
...
export default function ProductsPage() {
  ...
  const [formData, setFormData] = useState({ name: '', group_name: '', category_id: '', price: '', cost: '', unit: 'piece', source: 'baked', low_stock_threshold: '', is_active: true });
  ...
  const resetForms = () => {
    ...
    setFormData({ name: '', group_name: '', category_id: '', price: '', cost: '', unit: 'piece', source: 'baked', low_stock_threshold: '', is_active: true });
  ...
  const handleEditSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { ...formData, low_stock_threshold: formData.low_stock_threshold !== '' ? Number(formData.low_stock_threshold) : null };
          ...
          <div className="card-body"><div className="table-responsive"><table className="table table-hover"><thead><tr><th>ID</th><th>Variant</th><th>Category</th><th>Source</th><th>Price</th><th>Cost</th><th>Unit</th><th>Status</th><th>Actions</th></tr></thead><tbody>
            {variants.map((product) => (
              <tr key={product.id}>
                ...
                <td>
                  <button className="btn btn-sm btn-outline-primary me-2" onClick={() => { setEditingProduct(product); setFormData({ name: product.name, group_name: product.group_name || groupName, category_id: product.category_id || '', price: product.price, cost: product.cost || '', unit: product.unit || 'piece', source: product.source || 'baked', low_stock_threshold: product.low_stock_threshold ?? '', is_active: Boolean(product.is_active) }); }}><Edit size={14} /></button>
