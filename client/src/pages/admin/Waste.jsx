...
function getPeriodBounds(period, anchorDateValue = formatLocalDate(new Date())) {
  const safeAnchorDateValue = normalizeAnchorDate(anchorDateValue);
  const anchorDate = new Date(`${safeAnchorDateValue}T12:00:00`);
  const start = new Date(anchorDate);

  if (period === 'weekly') {
    start.setDate(anchorDate.getDate() - 6);
  } else if (period === 'monthly') {
    start.setDate(anchorDate.getDate() - 29);
  }

  return {
    start: formatLocalDate(start),
    end: safeAnchorDateValue,
  };
}
...
export default function WastePage() {
  ...
  const [summary, setSummary] = useState({ daily_loss: 0, weekly_loss: 0, monthly_loss: 0, total_loss: 0, monthly_items: 0, filtered_loss: 0, filtered_items: 0 });
  const [period, setPeriod] = useState('daily');
  const [anchorDate, setAnchorDate] = useState(() => normalizeAnchorDate(formatLocalDate(new Date())));
  ...
  const loadWasteData = async (selectedPeriod = period, selectedAnchorDate = anchorDate) => {
    setError('');
    const safeAnchorDate = normalizeAnchorDate(selectedAnchorDate);
    const bounds = getPeriodBounds(selectedPeriod, safeAnchorDate);
    ...
    try {
      const [rowsRes, summaryRes, expiringRes] = await Promise.all([
        ...
        api.get('/waste/summary', { params: { start_date: bounds.start, end_date: bounds.end } }),
  ...
  const periodBounds = useMemo(() => getPeriodBounds(period, normalizeAnchorDate(anchorDate)), [period, anchorDate]);
  const selectedLoss = Number(summary.filtered_loss || 0);
    ...
    <div className="waste-page">
      <div className="page-header waste-header">
        ...
        <div className="d-flex gap-2 flex-wrap">
          <button className="btn btn-outline-secondary" onClick={() => loadWasteData(period, anchorDate)}>
      ...
      <div className="card mb-3">
        <div className="card-body d-flex justify-content-between align-items-center flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted mb-0">Switch between daily, weekly, and monthly views, then anchor the report to a specific date.</p>
          </div>
          <div className="d-flex gap-2 flex-wrap align-items-center">
            ...
            <input
              ...
              style={{ minWidth: 180 }}
              value={anchorDate}
              onChange={(e) => setAnchorDate(normalizeAnchorDate(e.target.value))}
            />
          </div>
        </div>
      </div>
      ...
      <div className="stats-grid mb-4">
        <div className="stat-card card bg-light">
          ...
          <div className="stat-content">
            ...
            <small className="text-muted">{periodBounds.start} to {periodBounds.end} | {Number(summary.filtered_items || 0)} entries</small>
          </div>
        </div>
              ...
              {visibleWasteRows.map((row) => (
                <div className="waste-list-item" key={row.id}>
                  <div>
                    ...
                    <div className="waste-item-meta">
                      Wasted: {new Date(row.wasted_at).toLocaleString()} | {row.created_by_name || 'System'}
                      {row.expires_at ? ` | Expired: ${new Date(row.expires_at).toLocaleString()}` : ''}
                    </div>
                  </div>
