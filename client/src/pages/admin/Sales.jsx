...
export default function SalesPage() {
  ...
  return (
    <div className="sales-page">
      ...
      <div className="card sales-filter-card mb-4">
        <div className="card-body">
          <div className="sales-filter-head">
            <h5 className="mb-0 sales-filter-title">Filters</h5>
            <button className="btn btn-outline-secondary btn-sm sales-clear-btn" onClick={() => setFilters(initialFilters)}>
          ...
          <div className="sales-filter-grid">
            ...
            <div className="sales-filter-field sales-filter-field-wide">
              <label className="form-label">Search</label>
              <div className="input-group">
                <input
                  type="text"
                  className="form-control"
                  placeholder={activeView === 'sales' ? 'Receipt #, cashier, amount' : 'Batch #, creator'}
      ...
      {activeView === 'sales' ? (
        <>
          <div className="sales-metrics-grid mb-4">
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-success"><DollarSign size={20} /></div><div className="sales-metric-content"><h3>ETB {totalAmount.toFixed(2)}</h3><p>Total Sales</p></div></div>
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-primary"><CreditCard size={20} /></div><div className="sales-metric-content"><h3>{filteredSales.length}</h3><p>Total Transactions</p></div></div>
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-info"><Calendar size={20} /></div><div className="sales-metric-content"><h3>ETB {filteredSales.length > 0 ? (totalAmount / filteredSales.length).toFixed(2) : '0.00'}</h3><p>Avg. Transaction</p></div></div>
          </div>
          <div className="card sales-table-card"><div className="card-body"><div className="table-responsive"><table className="table table-hover"><thead><tr><th>Receipt #</th><th>Date & Time</th><th>Amount</th><th>Payment Method</th><th>Status</th><th>Void Details</th><th>Cashier</th><th>Actions</th></tr></thead><tbody>
            {filteredSales.length === 0 ? <tr><td colSpan="8" className="text-center text-muted py-4">No sales found</td></tr> : filteredSales.map((sale) => (
              <tr key={sale.id} className={sale.status === 'voided' ? 'sales-row-voided' : ''}><td>{sale.receipt_number}{sale.last_print_result === 'failed' ? <span className="badge badge-danger ms-2">Print issue</span> : null}</td><td>{formatAddisDateTime(sale.sale_date, { hour12: true })}</td><td>ETB {Number(sale.total_amount).toFixed(2)}</td><td><span className={`badge ${sale.payment_method === 'cash' ? 'badge-success' : sale.payment_method === 'card' || sale.payment_method === 'telebirr' ? 'badge-primary' : 'badge-info'}`}>{sale.payment_method}</span></td><td>{sale.is_offline ? <span className="badge badge-warning">Offline</span> : <span className="badge badge-success">Online</span>}<div className="small text-muted">Prints {sale.print_attempts || 0} · Reprints {sale.reprint_count || 0}</div></td><td>{sale.status === 'voided' ? <div><small className="text-muted d-block">{formatAddisDateTime(sale.voided_at, { hour12: true })}</small><small className="text-danger">{sale.void_reason || 'No reason provided'}</small></div> : <span className="text-muted">-</span>}</td><td>{sale.cashier_name || sale.cashier_id}</td><td><button className="btn btn-sm btn-outline-primary" onClick={() => openSaleDetails(sale)}><Eye size={14} /> View</button></td></tr>
            ))}
        ...
        <>
          <div className="sales-metrics-grid mb-4">
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-success"><DollarSign size={20} /></div><div className="sales-metric-content"><h3>ETB {totalBatchCost.toFixed(2)}</h3><p>Total Batch Cost</p></div></div>
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-primary"><Package size={20} /></div><div className="sales-metric-content"><h3>{batchActionCount}</h3><p>Total Actions</p></div></div>
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-info"><CreditCard size={20} /></div><div className="sales-metric-content"><h3>{sentBatchCount}</h3><p>Total Batches Sent</p></div></div>
            <div className="card sales-metric-card"><div className="sales-metric-icon tone-warning"><Calendar size={20} /></div><div className="sales-metric-content"><h3>{editedBatchCount}</h3><p>Total Batches Edited</p></div></div>
          </div>
          <div className="card sales-table-card"><div className="card-body"><div className="table-responsive"><table className="table table-hover"><thead><tr><th>Batch #</th><th>Created At</th><th>Created By</th><th>Status</th><th>Items</th><th>Total Cost</th><th>Sync</th><th>Details</th></tr></thead><tbody>
