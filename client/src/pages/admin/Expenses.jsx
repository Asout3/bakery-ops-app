...
export default function ExpensesPage() {
  ...
  return (
    <div className="expenses-page">
      ...
      <div className="card mb-4"><div className="card-body"><div className="row g-3 align-items-end"><div className="col-md-4"><label className="form-label" htmlFor="day-filter">Filter by Day</label><input id="day-filter" type="date" className="form-control" value={selectedDay} onChange={(e) => setSelectedDay(e.target.value)} aria-label="Filter expenses by day" /></div><div className="col-md-4">{selectedDay && <button className="btn btn-outline-secondary" onClick={() => setSelectedDay('')} title="Clear day filter" aria-label="Clear day filter">Clear Filter</button>}</div></div></div></div>
      ...
      <div className="card"><div className="card-body"><div className="table-responsive"><table className="table table-hover"><thead><tr><th>Expense ID</th><th>Date</th><th>Category</th><th>Description</th><th>Amount</th><th>Created By</th>{isAdmin && <th>Edit Window</th>}{isAdmin && <th>Actions</th>}</tr></thead><tbody>
        {expenses.length === 0 ? <tr><td colSpan={isAdmin ? 8 : 6} className="text-center">No expenses found</td></tr> : expenses.map((expense) => (
          <tr key={expense.id}><td>{expense.expense_code || `EXP-${String(expense.id).padStart(6, '0')}`}</td><td>{new Date(expense.expense_date).toLocaleDateString()}</td><td><span className="badge badge-primary">{expense.category}</span></td><td><button className="btn btn-sm btn-outline-secondary" onClick={() => setShowDescription(expense)} title="View description" aria-label={`View description for expense ${expense.expense_code || expense.id}`}><Eye size={14} /> View</button></td><td><strong>ETB {Number(expense.amount).toFixed(2)}</strong></td><td>{expense.created_by_name || '-'}</td>{isAdmin && <td>{isEditable(expense) ? <span className="badge badge-warning"><Clock size={12} className="me-1" />{getMinutesRemaining(expense)}m left</span> : <span className="badge badge-secondary">Locked</span>}</td>}{isAdmin && <td><button className="btn btn-sm btn-outline-primary me-2" onClick={() => { if (!isEditable(expense)) return; setEditingExpense(expense); setFormData({ category: expense.category, description: expense.description || '', amount: expense.amount, expense_date: expense.expense_date }); setShowForm(true); }} disabled={!isEditable(expense)} title="Edit expense" aria-label={`Edit expense ${expense.expense_code || expense.id}`}><Edit size={14} /></button><button className="btn btn-sm btn-outline-danger" onClick={() => handleDelete(expense.id)} disabled={!isEditable(expense)} title="Delete expense" aria-label={`Delete expense ${expense.expense_code || expense.id}`}><Trash2 size={14} /></button></td>}</tr>
        ))}
