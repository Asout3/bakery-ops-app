...
import { Plus, Minus, ShoppingCart, Trash2, Search, Banknote, Smartphone, Wallet, ZoomIn, ZoomOut } from 'lucide-react';
import './Sales.css';
...
const CART_ZOOM_LEVELS = ['compact', 'normal', 'comfortable'];
const CART_ZOOM_LABELS = {
  compact: 'Compact',
  normal: 'Normal',
  comfortable: 'Comfort',
};
...
export default function Sales() {
  ...
  const [cartZoomLevel, setCartZoomLevel] = useState(() => {
    const stored = localStorage.getItem('cashier_cart_zoom_level');
    return CART_ZOOM_LEVELS.includes(stored) ? stored : 'normal';
  });
  ...
  useEffect(() => {
    localStorage.setItem('cashier_cart_zoom_level', cartZoomLevel);
  }, [cartZoomLevel]);
  ...
  const availableGroups = useMemo(
    () => Array.from(
      new Set(products.filter((p) => p.is_active !== false).map((p) => p.group_name || p.name))
    ).sort(),
    [products],
  );
  ...
  const availableCategories = useMemo(
    () => Array.from(
      new Set(products.filter((p) => p.is_active !== false).map((p) => p.category_name || 'Uncategorized'))
    ).sort(),
    [products],
  );
  ...
  const cartZoomIndex = Math.max(0, CART_ZOOM_LEVELS.indexOf(cartZoomLevel));
  const canZoomOutCart = cartZoomIndex > 0;
  const canZoomInCart = cartZoomIndex < CART_ZOOM_LEVELS.length - 1;

  const updateCartZoom = (direction) => {
    setCartZoomLevel((current) => {
      const currentIndex = Math.max(0, CART_ZOOM_LEVELS.indexOf(current));
      const nextIndex = Math.min(CART_ZOOM_LEVELS.length - 1, Math.max(0, currentIndex + direction));
      return CART_ZOOM_LEVELS[nextIndex];
    });
  };

  const visibleVariantCount = groupedProducts.reduce((sum, [, variants]) => sum + variants.length, 0);
    ...
    <div className="sales-page">
      <div className="page-header sales-page-header">
        <div className="sales-title-wrap">
          <h2>{t('newSale')}</h2>
          <p className="sales-subtitle">Fast checkout workspace for products, variants, and receipts.</p>
        </div>
        <div className="sales-header-meta">
          <span className={`sales-status-pill ${isOnline ? 'sales-status-online' : 'sales-status-offline'}`}>
            {isOnline ? 'Online' : 'Offline'}
          </span>
          <span className="sales-meta-pill">{groupedProducts.length} groups</span>
          <span className="sales-meta-pill">{totalCartItems} items in cart</span>
        </div>
      </div>
      ...
      <div className="sales-layout">
        <div className="products-section">
          <div className="card">
            <div className="card-header sales-card-header">
              <div>
                <h3>Product Groups</h3>
                <p className="sales-card-subtitle">
                  {groupedProducts.length} groups • {visibleVariantCount} variants visible
                </p>
              </div>
            </div>
            ...
            <div className="card-body">
              <div className="filters-row">
                <div className="search-bar">
                  ...
                  <input
                    className="input"
                    placeholder="Search groups or variants..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
                </div>

                <select className="input filter-control filter-control-source" value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)}>
                  <option value="all">All Sources</option>
                  <option value="baked">Baked</option>
                  <option value="purchased">Purchased</option>
                </select>

                <select className="input filter-control" value={groupFilter} onChange={(e) => setGroupFilter(e.target.value)}>
                  <option value="all">All Groups</option>
                  {availableGroups.map((group) => <option key={group} value={group}>{group}</option>)}
                </select>

                <select className="input filter-control" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
                  <option value="all">All Categories</option>
                  {availableCategories.map((category) => <option key={category} value={category}>{category}</option>)}
                </select>

                <button
                  className="btn btn-outline-secondary filter-reset-btn"
                  type="button"
                  onClick={() => {
                    setSearchTerm('');
                    setSourceFilter('all');
              ...
              <div className="products-grid">
                {groupedProducts.map(([groupKey, variants]) => {
                  ...
                  const sourceTags = Array.from(new Set(variants.map((v) => String(v.source || 'baked'))));
                  const sourceLabel = sourceTags.length === 1 ? sourceTags[0] : 'mixed';
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (!isOutOfStock) handleGroupClick(groupKey, variants);
                      }}
                      role="button"
                      ...
                      onKeyDown={(e) => {
                        if (!isOutOfStock && (e.key === 'Enter' || e.key === ' ')) {
                          e.preventDefault();
                          handleGroupClick(groupKey, variants);
                        }
                      }}
                    >
                      <div className="product-card-head">
                        <div className="product-name">{groupKey}</div>
                        <div className="product-variant-pill">
                          {variants.length} variant{variants.length > 1 ? 's' : ''}
                        </div>
                      </div>

                      <div className="product-price">
                        ETB {min === max ? min.toFixed(2) : `${min.toFixed(2)} - ${max.toFixed(2)}`}
                      </div>
                      ...
                      <div className="product-card-meta">
                        ...
                        <div className="product-source-tag">{sourceLabel}</div>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="cart-section">
          <div className="card cart-card">
            <div className="card-header sales-card-header cart-card-header">
              <h3><ShoppingCart size={20} />Cart</h3>
              <div className="cart-header-actions">
                <div className="cart-zoom-controls" role="group" aria-label="Cart product zoom controls">
                  <button
                    type="button"
                    className="btn btn-sm btn-secondary cart-zoom-btn"
                    onClick={() => updateCartZoom(-1)}
                    disabled={!canZoomOutCart}
                    aria-label="Zoom out cart products"
                    title="Zoom out cart products"
                  >
                    <ZoomOut size={14} />
                  </button>
                  <span className="cart-zoom-label">{CART_ZOOM_LABELS[cartZoomLevel]}</span>
                  <button
                    type="button"
                    className="btn btn-sm btn-secondary cart-zoom-btn"
                    onClick={() => updateCartZoom(1)}
                    disabled={!canZoomInCart}
                    aria-label="Zoom in cart products"
                    title="Zoom in cart products"
                  >
                    <ZoomIn size={14} />
                  </button>
                </div>
                <span className="cart-count-pill">{totalCartItems} item{totalCartItems === 1 ? '' : 's'}</span>
              </div>
            </div>

            <div className={`card-body cart-body cart-body-${cartZoomLevel}`}>
              {cart.length === 0 ? (
                  ...
                  {cart.map((item) => (
                    <div key={item.product_id} className="cart-item">
                      <div className="cart-item-top">
                        ...
                        <div className="cart-item-subtotal">ETB {(item.price * item.quantity).toFixed(2)}</div>
                      </div>

                      <div className="cart-item-price">Unit ETB {Number(item.price).toFixed(2)}</div>

                      <div className="cart-item-actions">
                        <button className="btn btn-sm btn-secondary cart-action-btn" onClick={() => updateQuantity(item.product_id, -1)}>
                          <Minus size={14} />
                        </button>
                        ...
                        <input
                          type="number"
                          min="1"
                          max={Math.max(1, Number(products.find((p) => Number(p.id) === Number(item.product_id))?.stock_quantity || 0))}
                          className="form-control form-control-sm cart-qty-input"
                          value={quantityDrafts[item.product_id] ?? String(item.quantity)}
                          onChange={(e) => handleQuantityDraftChange(item.product_id, e.target.value)}
                          onBlur={(e) => setQuantity(item.product_id, e.target.value)}
                        />
                        ...
                        <button className="btn btn-sm btn-danger cart-action-btn" onClick={() => removeFromCart(item.product_id)}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
            ...
            <div className="card-footer cart-footer">
              <div className="payment-method-select">
                ...
                <div className="payment-method-toggle" role="radiogroup" aria-label="Payment method">
                  {paymentOptions.map((option) => {
                    ...
                    return (
                      <button
                        key={option.value}
                        type="button"
                        className={`payment-method-btn payment-method-btn-${option.value} ${paymentMethod === option.value ? 'payment-method-btn-active' : ''}`}
              ...
              <div className="cart-total">
                <span className="cart-total-label">Total ({totalCartItems} items)</span>
                <span className="cart-total-amount">ETB {calculateTotal().toFixed(2)}</span>
              </div>

              <p className="cart-footer-note">
                {isOnline ? 'Connected: sales are saved instantly.' : 'Offline: sales will queue and sync automatically.'}
              </p>
