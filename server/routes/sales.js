  ...
  try {
    if (receiptEnabled) {
      ...
      if (receiptSettings.enabled !== false) {
        const resolvedTemplate = normalizeReceiptTemplateSchema(receiptTemplateSnapshot || activeReceiptTemplate?.schema || {});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const locationId = await getTargetLocationId(req, query);
      const { settings: receiptSettings, activeTemplate: activeReceiptTemplate } = await getReceiptConfig(locationId || null);
      const receiptSystemEnabled = receiptSettings.enabled !== false;
      const saleExecution = await withTransaction(async (tx) => {
        ...
        const receiptEnabled = receiptSystemEnabled && receipt_enabled !== false;
