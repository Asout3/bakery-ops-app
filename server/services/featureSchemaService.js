...
async function ensureOrdersSchema() {
  ...
  await query('CREATE INDEX IF NOT EXISTS idx_customer_orders_location_pickup ON customer_orders(location_id, pickup_at)');
  await query('CREATE INDEX IF NOT EXISTS idx_customer_orders_status_pickup ON customer_orders(status, pickup_at)');
  await query('CREATE INDEX IF NOT EXISTS idx_customer_orders_created_at ON customer_orders(created_at)');
...
async function ensureArchiveSchema() {
  ...
  await query('CREATE INDEX IF NOT EXISTS idx_staff_payments_archive_payment_date ON staff_payments_archive(payment_date)');

  archiveSchemaReady = true;
}

export async function ensureFeatureSchema({ orders = false, archive = false } = {}) {
  if (orders) {
